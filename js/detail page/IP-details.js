...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    // --- 個別JSをロード ---
    loadAnimeScript(anime.id, anime);
...
// 個別JSを読み込む関数
function loadAnimeScript(animeId, animeData) {
  const script = document.createElement("script");
  script.src = `./js/detail%20page/IP/${animeId}.js`;  
  // 例: ./js/detail%20page/IP/anime-001.js
  script.defer = true;
  script.onload = () => {
    console.log(`✅ ${animeId}.js 読み込み完了`);
    if (typeof initAnimeCustom === "function") {
      initAnimeCustom(animeData); 
      // animeData を渡せば個別JSで自由に使える
    }
  };
  script.onerror = () => {
    console.warn(`⚠️ ${animeId}.js が見つかりませんでした`);
  };
  document.body.appendChild(script);
}


//{以下はいままで使ってたjs（保存しているだけ）
//document.addEventListener("DOMContentLoaded", async () => {
//  const container = document.getElementById("anime-detail");
...
// 個別JSを読み込む関数
//function loadAnimeScript(animeId) {
//  const script = document.createElement("script");
//  script.src = `./scripts/${animeId}.js`;  // 例: ./scripts/worldtrigger-001.js
//  script.defer = true;
//  script.onload = () => {
//    console.log(`✅ ${animeId}.js 読み込み完了`);
//    if (typeof initAnimeCustom === "function") {
//      initAnimeCustom(); // 個別JSに書く初期化関数を呼び出す
//    }
//  };
//  script.onerror = () => {
//    console.warn(`⚠️ ${animeId}.js が見つかりませんでした`);
